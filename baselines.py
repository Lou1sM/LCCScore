...
def im_compressed_size(im, comp_format):
    x = im - im.min()
    x = im/im.max()
    ...
    compressed = BytesIO()
    pim.save(compressed,format=comp_format)
    return compressed.getbuffer().nbytes

def im_compression_ratio(im, comp_format):
    #full_size = 8 * im.size
    full_size = im.nbytes
    compressed_size = im_compressed_size(im, comp_format)
